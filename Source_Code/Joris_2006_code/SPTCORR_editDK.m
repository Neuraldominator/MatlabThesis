function [x, BC, NC] = SPTCORR_editDK(spt1, spt2, maxlag, binwidth, dur, normStr)
...
% Nspike1: number of spikes in first spike train
% Nspike2: number of spikes in second spike train
...
NC = CollectInStruct(Nspike1, Nspike2, Rate1, Rate2, Nrep1, Nrep2, dur, ...
    binwidth, NF, LouageNorm);

%--------------------------------------------------------------------------
function x = localApplyNorm(x, NC, normStr)
%Apply normalization
if ~isempty(normStr)
 if ~isfield(NC, normStr) % returns true if normStr is the name of a field in the structure array NC.
