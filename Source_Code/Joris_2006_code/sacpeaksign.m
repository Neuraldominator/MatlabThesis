function [p, BinCenters, Ppdf, Pcdf, Nco, Spt_Nco] = sacpeaksign(Spt, CorBinWidth, NTrials, dur)
% SACPEAKSIGN - SAC peak significance
%   p = SACPeakSign(Spt, BinWidth, NTrials, Dur) evaluates the statistical 
%   significance of the peak value of the shuffled autocorrelogram (SAC; 
%   see SPTCORR) derived from spiketrain collection Spt. 
%   Input args:
%     Spt:          cell array of spike time vectors; each vector is "repetition"
%     CorBinWidth:  width of coincidence window
...
%
%   [p, BinCenters, Ppdf, Pcdf, Nco] = SACPeakSign(Spt, BinWidth, NTrials) 
%   also returns the probability density function(PDF) and the cumulative 
%   distribution function(CDF) of the Nco values as estimated from the 
%   trials. BinCenters contains the bin centers of these functions, e.g., 
%   BAR(Nco,Ppdf) will produce a graph of the pdf.
%
%   See also SPTCORR.
...
elseif ~iscell(Spt) | ~all(cellfun('isclass', Spt, 'double')), ...
        error('First argument should be cell array of vectors containing spiketimes.');
elseif ~isnumeric(CorBinWidth) | (length(CorBinWidth) ~= 1) | (CorBinWidth < 0), ...
        error('Second argument should be positive scalar specifying correlation binwidth.');
elseif ~isnumeric(NTrials) | (length(NTrials) ~= 1) | (NTrials <= 0), ...
        error('Third argument should be positive scalar specifying number of trials.'); 
end
...
for n = 1:NTrials
 ...
 for i = 1:NRep
   ScrSpt{i} = ScrambleSpkTr(Spt{i}, dur); 
 end
 %Calculate the number of coincidences at zero delay ...
 Nco(n) = SPTCORR(ScrSpt, 'nodiag', 0, CorBinWidth);
end

Edges      = linspace(min(Nco), max(Nco), NTrials); % generates a row vector of 'NTrials' equally spaced values between min(Nco) and max(Nco) 
BinWidth   = diff(Edges([1 2])); 
BinCenters = Edges([1:end-1])+BinWidth/2;
Npdf       = histc(Nco, Edges);
Npdf(end)  = [];
Ppdf       = Npdf/NTrials; 
Pcdf       = cumsum(Ppdf);
