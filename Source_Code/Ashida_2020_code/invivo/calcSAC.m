...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculating shuffled autocorrelogram of spike trains (ver 0.9 by GA)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input 
%  SPin: 1-D cell array of spike time vectors in ms 
...
%  CN   : normalization factor = (#reps) * (#reps-1) * (rate^2) * (bin width) * (time length) 
...
% normalization
T = T2-T1; % [ms] time length
R = Nsp / (Nreps*T); % [spikes/ms] average spike rate
CN = Nreps * (Nreps-1) * R^2 * BW * T; % normalization factor
SAC = SAC/CN; % final results 
CI = SAC(Nbins+1); 
